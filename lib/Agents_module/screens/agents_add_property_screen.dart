...
class _AgentsAddPropertyScreenState extends State<AgentsAddPropertyScreen> {
  ...
  _addAgentPropertyHandler()async{
    CustomLoader.showLoader(context: context);
    await AgentsAddPropertyService().addAgentProperty(context: context, propTitle: _propTitleCont.text, content: _contentCont.text, propertyType: selectedProperty, purpose: selectedPurpose, price: _priceCont.text, landArea: _landAreaCont.text, unit: selectedUnit, noOfBeds: _noOfBathsCont.text, noOfBaths: _noOfBathsCont.text, expiryDate: selectedExpiration, city: _selectedCity!, area:selectedArea!, detailsAddress: _addressCont.text);
    CustomLoader.hideLoader(context);
  }
  @override
